...
#flags.DEFINE_string("name", default="rnn-t-v5", help="session name")
flags.DEFINE_string("model_name", default="last.pt", help="steps of checkpoint")
...
def main(argv):
    ...
    #flagFile = flags.FLAGS.flagfile
    #sessionName = flags.FLAGS.name
    #modelName = flags.FLAGS.model_name
    flags.FLAGS(["stream.py", "--flagfile=./flagfiles/E6D2_LARGE_Batch.txt", "--name=rnnt-m-bpe", "--model_name=english_43_medium.pt", "--step_n_frame=2"])
    #flagFile = flags.FLAGS.flagfile
    #print(argv[0:])
    stream_decoder = PytorchStreamDecoder(flags.FLAGS)
    duration = 80
...
if __name__ == "__main__":
    flags.mark_flag_as_required("flagfile")
